...
public class BarChartView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (source != null && source.size() > 0)
        {
            ...
            float gapSize = 0.2f * widthOfElement;
            float widthOfBar = widthOfElement - gapSize;
...
//                        1 + random.nextInt(254));
                ...
                float x1 = paddW + i * widthOfElement + gapSize;
                float y1 = (float) ((1 - source.get(labels.get(i)) / maxValue) * availableHeight + paddH);
                float x2 = x1 + widthOfBar;
                float y2 = paddH + availableHeight - 5;
